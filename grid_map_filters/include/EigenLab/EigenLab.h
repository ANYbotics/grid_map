...
#ifndef EIGENLAB__EIGENLAB_H_
#define EIGENLAB__EIGENLAB_H_
...
public:
    ...
    inline Value & operator = (const Value & val) {if (val.isLocal()) {
        setLocal(val);
      } else {
        setShared(val);
      } return *this;}
...
private:
    ...
    struct Chunk
    {
      ...
      Chunk(
        ...
        const Value < Derived > & val = Value < Derived > ()) : field(str), type(t), value(val),
