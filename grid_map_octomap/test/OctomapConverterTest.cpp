...
{
  GridMapOctomapConverter octomap_converter;
  ...
  // Convert to grid map.
  GridMap gridmap;
  bool res = octomap_converter.fromOctomap(octomap, gridmap);
  ASSERT_TRUE(res);
...
TEST(OctomapConversion, convertOctomapToGridMapWithBoundingBox)
{
  ...
  bool res = octomap_converter.fromOctomap(octomap, gridmap, &minpt, &maxpt);
  ASSERT_TRUE(res);
  Length length = gridmap.getLength();
