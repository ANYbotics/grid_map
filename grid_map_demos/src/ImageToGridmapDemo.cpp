...
{
  readParameters();
  imageSub_ = nodeHandle_.subscribe(imageTopic_, 1,
                                    &ImageToGridmapDemo::imageCallback, this);
  gridMapPublisher_ = nodeHandle_.advertise<grid_map_msgs::GridMap>("grid_map",
                                                                    1, true);
}
...
void ImageToGridmapDemo::imageCallback(const sensor_msgs::Image& msg)
{
  if (!mapInitialized_) {
    ...
    grid_map::GridMapRosConverter::fromImage(msg, std::string("mask"),
                                             mapLengthX_, map_);
  ...
  grid_map::GridMapRosConverter::addLayerFromGrayscaleImage(
      msg, std::string("elevation"), map_, minHeight_, maxHeight_);
