...
TEST(GridMap, TransformRotate90) {
  ...
  Eigen::Isometry3d transform =
    Eigen::Translation3d(0, 0, 0) * Eigen::AngleAxisd(M_PI / 2, grid_map::Vector3::UnitZ());

  // Apply affine transformation.
  const grid_map::GridMap transformedMap =
    grid_map::GridMapCvProcessing::getTransformedMap(grid_map::GridMap(map),
                                                      transform,
                                                      heightLayerName,
                                                      map.getFrameId());
...
TEST(GridMap, TransformRotate180) {
  ...
  Eigen::Isometry3d transform =
    Eigen::Translation3d(0, 0, 0) * Eigen::AngleAxisd(M_PI, grid_map::Vector3::UnitZ());
...
TEST(GridMap, TransformRotate45) {
  ...
  map.setGeometry(grid_map::Length(1.0, 2.0), resolution, grid_map::Position(0.0, 0.0));
  map.add(heightLayerName, 1.0);
  map.setBasicLayers(map.getLayers());

  // Transformation (45° rotation).
  Eigen::Isometry3d transform =
    Eigen::Translation3d(0, 0, 0) * Eigen::AngleAxisd(M_PI / 4, grid_map::Vector3::UnitZ());
  ...
  EXPECT_NEAR(transformedMap.get(heightLayerName).sumOfFinites() /
              map.get(heightLayerName).size(), 1.0, 1e-2);
}
...
TEST(GridMap, TransformTranslationXZY) {
  ...
  Eigen::Isometry3d transform =
    Eigen::Translation3d(1, -0.5, 0.2) * Eigen::AngleAxisd(0, grid_map::Vector3::UnitZ());
  ...
  EXPECT_NEAR(map.get(heightLayerName)(0, 0) + transform.translation().z(),
              transformedMap.get(heightLayerName)(0, 0), 1e-6);
...
TEST(GridMap, TransformUnaligned) {
  ...
  Eigen::Isometry3d transform =
    Eigen::Translation3d(0, 0, 0) * Eigen::AngleAxisd(M_PI / 8, grid_map::Vector3::UnitY());

  // Check that this unaligned transformation is not accepted.
  ASSERT_ANY_THROW(const grid_map::GridMap transformedMap =
    grid_map::GridMapCvProcessing::getTransformedMap(grid_map::GridMap(map),
                                                      transform,
                                                      heightLayerName,
                                                      map.getFrameId()));
...
TEST(GridMap, TransformComputationTime) {
  ...
  grid_map::GridMap map;
  const auto heightLayerName = "height";
  map.setGeometry(grid_map::Length(4.0, 4.0), 0.02, grid_map::Position(0.0, 0.0));
  map.add(heightLayerName, 0.0);
  map.setBasicLayers(map.getLayers());

  // Translation by (1, -0.5, 0.2):
  Eigen::Isometry3d transform =
    Eigen::Translation3d(1, -0.5, 0.2) * Eigen::AngleAxisd(M_PI / 4, grid_map::Vector3::UnitZ());

  // Setup timing metrics.
  int64_t sumTimes = 0;
  int64_t averageTime = 0;
  int64_t minTime = std::numeric_limits<int64_t>::max();
  int64_t maxTime = std::numeric_limits<int64_t>::lowest();
  ...
  for (size_t sampleNumber = 0; sampleNumber < numberOfSamples; sampleNumber++) {
    ...
    int64_t time = std::chrono::duration_cast<std::chrono::nanoseconds>(timestampAfter -
     timestampBefore).count();
    sumTimes += time;
    if (time < minTime) {
      minTime = time;
    }
  ...
  std::cout << "Transforming a 2 x 2 m GridMap with a resolution of 2 cm by 45° took: "
            << std::endl
            << "avg: " << averageTime * 1e-6f << "ms, \tmin: " << minTime * 1e-6f << "ms, \tmax "
            << maxTime * 1e-6f << "ms." << std::endl;
}
