...
GridMap GridMap::convertResolution(const double resolution)
{
  if (resolution == resolution_) return *this;
  // Generate new map.
  GridMap map(layers_);
  map.setBasicLayers(basicLayers_);
  map.setTimestamp(timestamp_);
  map.setFrameId(frameId_);
  // Set resolution.
  map.setGeometry(length_, resolution, position_);
  map.startIndex_.setZero();

//  map.fillHolesFrom(*this);
  return map;
}
